...
;(function () {
  ...
  // 在飞跃站点：只「自报已安装」，供学分统计页的导入向导检测；不注入按钮。
  var HOST = location.hostname
  if (HOST === 'feiyue.selab.top' || HOST === 'winbeau.top') {
    try {
      window.__feiyueImporterReady = true
      document.documentElement.setAttribute('data-feiyue-importer', 'extension')
  ...
  var C = {
    bg: '#ffffff', bgHover: '#f7f6f3', text: '#37352f', muted: '#787774', line: '#dcdad4',
    green: '#0f7b6c', greenBg: 'rgba(15,123,108,.08)', greenLine: 'rgba(15,123,108,.4)',
  ...
  btn.addEventListener('click', function () {
    ...
    fetch(PDF_API + '?method=topdf', {
      ...
      .catch(function (e) { render('error', IC_CROSS, (e && e.message ? e.message : String(e))); reset(6000) })
  })
